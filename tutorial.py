...
pygame.display.set_caption("ANUSHA'S GAME PLATFORM")
...
# map : returns  inte or float of a string without necessarily want to convert each element in the string
...
#__init__ - The __init__ function is called every time an object is created from a class. 
# The __init__ method lets the class initialize the object's attributes

# __main__ - top level code

#__name__ built-in var, records the name of the currently running module or script.
...
class Player(pygame.sprite.Sprite): #sprite has properties we need without having to write each and everything
    COLOR = (255, 0, 0)
    GRAVITY = 1
    ...
    def move_left(self, vel):
        self.x_vel = -vel
        if self.direction != "left": #ratish
            self.direction = "left"
            self.animation_count = 0
    
    def move_right(self, vel):
        self.x_vel = vel
        if self.direction != "right": #ratish
...
def handle_vertical_collision(player, objects, dy):
    collided_objects = []
    for obj in objects:
        if pygame.sprite.collide_mask(player, obj): #if my player is colliding with oj
            if dy > 0:
...
def main(window):
    ...
    run = True
    while run:
        clock.tick(FPS) #ensures our while loop runs 60 frames per sec
...
if __name__ == "__main__": #only call the main fn if we run this file directly.. if we don't run, we imported something from this and don't run directly..
    main(window)
